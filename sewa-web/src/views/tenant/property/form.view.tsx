...
const Form = ({
  ...
  const onSubmit = (values: PropertyType) => {
    ...
    if (id) {
      updateMutation.mutate({ id: id, formData: formData });
    } else {
  ...
  useEffect(() => {
    if (initialValue) {
      ...
      let initialImages = initialValue?.images.map((datum: any) => {
        return { id: datum.id, filename: datum.filename };
      });
